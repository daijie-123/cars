...
//已登陆转向
if (is_user_login()) {
	redirect('','?mod=user&ac=index');
}

if (submitcheck('username'))
{
    $arr_not_empty = array('username'=>'用户名不能为空','password'=>'密码不能为空','repassword'=>'请再次输入密码','nicname'=>'请填写联系人','mobilephone'=>'请填写手机号','email'=>'电子邮箱不能为空');
    ...
    if (!$rs) {
		...
		showmsg('登陆成功','/?mod=user&ac=index');
	}
    showmsg('注册成功','/?mod=login');
}
